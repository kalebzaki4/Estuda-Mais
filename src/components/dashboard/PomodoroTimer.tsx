...
export default function PomodoroTimer({ 
  ...
  const handleAddTopic = (e) => {
    ...
    const hasMinLength = sanitized.length >= 10
    const hasAtLeastTwoWords = /\S+\s+\S+/.test(sanitized)
    const hasFiveConsecutiveRepeatedChars = /(.)\1{4,}/.test(sanitized)

    if ((!hasMinLength && !hasAtLeastTwoWords) || hasFiveConsecutiveRepeatedChars) {
